...
| `td_trace(path, depth)` | **Why is this TOP black?** Walks up every wired input from `path` (`depth` hops, at most 40 nodes), reads each image's min / mean / max over R, G, B and the alpha mean, and marks where the signal was lost: `dropped here` (the image goes black while an input still carried something, or an Add is fed a negative input and subtracts it), `negative values start here` (a float Level with black level above 0 and no clamp) or `NaN/Inf start here`, with the reason when the node's settings show it — bypass, opacity 0, an empty input, an error. `?` on a mark means an input could not be read. It cooks nothing: a node that never cooked, a non-TOP, or one past the time or download budget says so instead of reporting zeros. Wires only — an image a Select or Render TOP reaches through a parameter is not followed. |
