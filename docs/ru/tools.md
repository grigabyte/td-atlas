...
kommit: 0a1fec2
hesh: 8a5362c4341b23f0470b71474b01026013e713a9e0e41771e7388a036ed2b796
---
...
| `td_trace(path, depth)` | **Почему этот TOP чёрный?** Идёт вверх по всем проводным входам от `path` (`depth` шагов, не больше 40 узлов), снимает у каждой картинки мин / среднее / макс по R, G, B и среднее альфы и отмечает, где пропал сигнал: `dropped here` (картинка чернеет, хотя вход ещё что-то нёс, или в Add приходит отрицательный вход и вычитается), `negative values start here` (Level во float с black level выше 0 без клампа), `alpha goes to 0 here` (цвет под альфой 0) или `NaN/Inf start here` — с причиной, если её видно из настроек узла: bypass, opacity 0, пустой вход, ошибка. `?` у пометки значит, что вход прочитать не удалось. Ничего не кукает: узел, который ни разу не кукал, не-TOP или узел за бюджетом времени либо скачивания так и называется, а не показывает нули. Только провода: картинка, которую Select или Render TOP берёт через параметр, не прослеживается. |
