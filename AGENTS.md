...
Twenty-nine, in four groups. The CLI's live surface is the few commands a
...
| `render` / `td_render` | `save_to` is optional in the tool, where the CLI always writes a file (render.png unless told otherwise); `width` defaults to 512 in the tool and to the TOP's own resolution in the CLI; `overwrite` is MCP only | The tool's default answer is the image inline, where every pixel costs context, so it writes a file only when asked; the CLI writes a file, where the artist's own resolution is the right answer. The tool refuses a file already at `save_to` unless told to replace it, since an agent cannot see what is there; the CLI replaces it, like `project text -o` and a shell's `>`, since a person names the file in the directory in front of them and rerunning into `render.png` is the command's ordinary loop. |
